...
use ipinfo\ipinfo\IPinfoPlus as IPinfoPlusClient;
use ipinfo\ipinfolaravel\iphandler\IPHandlerInterface;
use ipinfo\ipinfolaravel\plus\ipinfopluslaravel;

class IpinfopluslaravelTest extends TestCase
{
    protected function getPackageProviders($app)
    {
        return [
            \ipinfo\ipinfolaravel\plus\ipinfopluslaravelServiceProvider::class,
        ];
    }
    ...
    protected function makeMiddleware(
        ...
        $mw = $this->getMockBuilder(ipinfopluslaravel::class)
    ...
    public function test_handle_merges_details_on_success()
    {
        ...
        $client = $this->createMock(IPinfoPlusClient::class);
        $client
            ->expects($this->once())
            ->method("getDetails")
            ->with("8.8.8.8")
